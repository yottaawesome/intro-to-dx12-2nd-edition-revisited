...
public:
    ...
    void Update(const GameTimer& gt)override
    {
        ...
        auto proj = DirectX::XMLoadFloat4x4(&mProj);
    ...
    void BuildConstantBuffers()
    {
        ...
        for (auto cbObjIndex = 0u; cbObjIndex < BOX_COUNT; ++cbObjIndex)
        {
            ...
            auto cbvObj = D3D12::D3D12_CONSTANT_BUFFER_VIEW_DESC{.BufferLocation = objCBAddress, .SizeInBytes = objCBByteSize};
        ...
        auto cbPassElementOffset = 0;
        auto passCBAddress = D3D12_GPU_VIRTUAL_ADDRESS{mPassCB->Resource()->GetGPUVirtualAddress() +
            cbPassElementOffset * passCBByteSize};

        auto cbvPassDesc = D3D12_CONSTANT_BUFFER_VIEW_DESC{.BufferLocation = passCBAddress, .SizeInBytes = passCBByteSize};
    ...
    void BuildBoxGeometry(ID3D12Device* device, DirectX::ResourceUploadBatch& uploadBatch)
    {
        ...
        constexpr auto indices = std::array<std::uint16_t, 36>{
            // front face
            0, 1, 2,
            0, 2, 3,
        ...
        constexpr auto ibByteSize = (UINT)indices.size() * sizeof(uint16_t);
