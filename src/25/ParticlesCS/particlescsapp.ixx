...
export module particlescs:particlescsapp;
import std;
import shared;
...
export class ParticlesCSApp : public D3DApp
{
public:
    ParticlesCSApp(HINSTANCE hInstance)
        : D3DApp(hInstance)
    {
    ...
    ParticlesCSApp(const ParticlesCSApp& rhs) = delete;
    ParticlesCSApp& operator=(const ParticlesCSApp& rhs) = delete;
    ~ParticlesCSApp()
    {
...
private:
    ...
    void UpdateMaterialBuffer(const GameTimer& gt)
    {
        ...
        for (auto& e : matLib.GetCollection())
        {
            ...
            DirectX::XMStoreFloat4x4(&matData.MatTransform, XMMatrixTranspose(matTransform));
    ...
    void EmitExplosionParticles(const GameTimer& gt)
    {
        ...
        auto spawnPos = MathHelper::Vector3{mWorldRayPos + mWorldRayDir * MathHelper::RandF(5.0f, 20.0f)};
        auto numParticlesEmitted = MathHelper::Rand(2000, 3000);
        ...
        auto explosionParticles = ParticleEmitCB{
            ...
            .gDragScale = 0.75f,
            .gEmitCount = static_cast<std::uint32_t>(numParticlesEmitted),
    ...
    void LoadGeometry()
    {
        ...
        auto columnSquareBroken = std::unique_ptr<MeshGeometry>{d3dUtil::LoadSimpleModelGeometry(
        ...
        auto columnRound = std::unique_ptr<MeshGeometry>{d3dUtil::LoadSimpleModelGeometry(
            md3dDevice.Get(), *mUploadBatch.get(), "Models/columnRound.m3d", "columnRound")};
        mGeometries[columnRound->Name] = std::move(columnRound);

        auto columnRoundBroken = std::unique_ptr<MeshGeometry>{d3dUtil::LoadSimpleModelGeometry(
        ...
        auto orbBase = std::unique_ptr<MeshGeometry>{d3dUtil::LoadSimpleModelGeometry(
    ...
    void BuildCommandSignatures()
    {
        ...
        auto indirectDispatchArgs = std::array{ D3D12::D3D12_INDIRECT_ARGUMENT_DESC{ .Type = D3D12::D3D12_INDIRECT_ARGUMENT_TYPE::D3D12_INDIRECT_ARGUMENT_TYPE_DISPATCH} };

        auto indirectDispatchDesc = D3D12::D3D12_COMMAND_SIGNATURE_DESC{
            .ByteStride = sizeof(D3D12::D3D12_DISPATCH_ARGUMENTS),
            .NumArgumentDescs = static_cast<std::uint32_t>(indirectDispatchArgs.size()),
        ...
        auto indirectDrawIndexedArgs = std::array{
			D3D12::D3D12_INDIRECT_ARGUMENT_DESC{ .Type = D3D12_INDIRECT_ARGUMENT_TYPE_DRAW_INDEXED }
        };
        auto indirectDrawIndexedDesc = D3D12::D3D12_COMMAND_SIGNATURE_DESC{
            .ByteStride = sizeof(D3D12::D3D12_DRAW_INDEXED_ARGUMENTS),
            .NumArgumentDescs = static_cast<std::uint32_t>(indirectDrawIndexedArgs.size()),
