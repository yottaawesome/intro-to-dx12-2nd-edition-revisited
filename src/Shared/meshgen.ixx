...
public:
    ...
    auto CreateSphere(float radius, std::uint32_t sliceCount, std::uint32_t stackCount) -> MeshGenData
    {
        ...
        auto thetaStep = 2.0f * DirectX::TwoPi / sliceCount;
    ...
    auto CreateGeosphere(float radius, std::uint32_t numSubdivisions) -> MeshGenData
    {
        ...
        for (auto i = 0u; i < meshData.Vertices.size(); ++i)
        {
            ...
            meshData.Vertices[i].TexC.x = theta / DirectX::TwoPi;
            meshData.Vertices[i].TexC.y = phi / DirectX::Pi;
...
private:
    ...
    void BuildCylinderBottomCap(float bottomRadius, float topRadius, float height, uint32_t sliceCount, uint32_t stackCount, MeshGenData& meshData)
    {
        ...
        // vertices of ring
        auto dTheta = float{ 2.0f * DirectX::TwoPi / sliceCount };
        for (auto i = 0u; i <= sliceCount; ++i)
        {
