...
export class TexturedShapesApp : public D3DApp
{
public:
...
private:
    ...
    void OnMouseMove(WPARAM btnState, int x, int y)override
    {
        ...
        if ((btnState & Win32::MK::LButton) != 0)
        {
            ...
            mPhi = std::clamp(mPhi, 0.1f, DirectX::Pi - 0.1f);
        }
    ...
    void UpdateMaterialBuffer(const GameTimer& gt)
    {
        ...
        for (auto& e : matLib.GetCollection())
        {
            ...
            auto matData = MaterialData{
                ...
                .DiffuseMapIndex = static_cast<unsigned>(mat->AlbedoBindlessIndex),
                .NormalMapIndex = static_cast<unsigned>(mat->NormalBindlessIndex),
                .GlossHeightAoMapIndex = static_cast<unsigned>(mat->GlossHeightAoBindlessIndex)
            };
    ...
    void BuildRenderItems()
    {
        ...
        DirectX::XMStoreFloat4x4(&texTransform, DirectX::XMMatrixScaling(1.0f, 0.5f, 1.0f));
        AddRenderItem(RenderLayer::Opaque, worldTransform, texTransform, matLib["bricks0"], mGeometries["shapeGeo"].get(), mGeometries["shapeGeo"]->DrawArgs["box"]);
        ...
        AddRenderItem(RenderLayer::Opaque, worldTransform, texTransform, matLib["skullMat"], mGeometries["skullGeo"].get(), mGeometries["skullGeo"]->DrawArgs["skull"]);

        worldTransform = MathHelper::Identity4x4;
        DirectX::XMStoreFloat4x4(&texTransform, DirectX::XMMatrixScaling(8.0f, 8.0f, 1.0f));
        AddRenderItem(RenderLayer::Opaque, worldTransform, texTransform, matLib["tile0"], mGeometries["shapeGeo"].get(), mGeometries["shapeGeo"]->DrawArgs["grid"]);

        for (int i = 0; i < 5; ++i)
        {
            DirectX::XMStoreFloat4x4(&texTransform, DirectX::XMMatrixScaling(1.5f, 2.0f, 1.0f));
            DirectX::XMStoreFloat4x4(&worldTransform, DirectX::XMMatrixTranslation(-5.0f, 1.5f, -10.0f + i * 5.0f));
            AddRenderItem(RenderLayer::Opaque, worldTransform, texTransform, matLib["bricks0"], mGeometries["shapeGeo"].get(), mGeometries["shapeGeo"]->DrawArgs["cylinder"]);
            ...
            AddRenderItem(RenderLayer::Opaque, worldTransform, texTransform, matLib["rock0"], mGeometries["shapeGeo"].get(), mGeometries["shapeGeo"]->DrawArgs["sphere"]);
