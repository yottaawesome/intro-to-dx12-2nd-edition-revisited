...
export class VecAddCS : public D3DApp
{
public:
    VecAddCS(Win32::HINSTANCE hInstance)
        : D3DApp(hInstance)
    {
        Initialize();
    }
    VecAddCS(const VecAddCS& rhs) = delete;
    VecAddCS& operator=(const VecAddCS& rhs) = delete;
    ~VecAddCS()
    {
...
private:
    ...
    void BuildRootSignature()
    {
        ...
        // Root parameter can be a table, root descriptor or root constants.
        auto computeRootParameters = std::array<CD3DX12_ROOT_PARAMETER, COMPUTE_ROOT_ARG_COUNT>{};
    ...
    void BuildShadersAndInputLayout()
    {
        if constexpr (IsDebugBuild)
        {
            ...
            mShaders["vecAddCS"] = d3dUtil::CompileShader(L"Shaders\\VecAdd.hlsl", { L"-E", L"CS", L"-T", L"cs_6_6", DXC::ArgDebug, DXC::ArgSkipOptimizations });
        }
        else
        {
            ...
            mShaders["vecAddCS"] = d3dUtil::CompileShader(L"Shaders\\VecAdd.hlsl", { L"-E", L"CS", L"-T", L"cs_6_6" });
        }
    ...
    void BuildPSOs()
    {
        ...
        auto computePsoDesc = D3D12::D3D12_COMPUTE_PIPELINE_STATE_DESC{};
        computePsoDesc.pRootSignature = mComputeRootSignature.Get();
        computePsoDesc.CS = {
            reinterpret_cast<Win32::BYTE*>(mShaders["vecAddCS"]->GetBufferPointer()), mShaders["vecAddCS"]->GetBufferSize()
        };
    ...
    ) -> std::unique_ptr<MeshGeometry>
    {
        ...
        constexpr auto indexElementByteSize = std::uint32_t{ sizeof(uint16_t) };
    ...
    {
        // Generate some data.
        auto dataA = std::vector<Data>(NumDataElements);
        auto dataB = std::vector<Data>(NumDataElements);
        for (int i = 0; i < NumDataElements; ++i)
        {
            auto x = static_cast<float>(i);
            dataA[i].v1 = DirectX::XMFLOAT3(x, x, x);
            dataA[i].v2 = DirectX::XMFLOAT2(x, 0);

            dataB[i].v1 = DirectX::XMFLOAT3(-x, x, 0.0f);
            dataB[i].v2 = DirectX::XMFLOAT2(0, -x);
        }

        auto byteSize = dataA.size() * sizeof(Data);
        ...
        auto buffer = D3D12::CD3DX12_RESOURCE_DESC::Buffer(byteSize, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
    ...
    void DoComputeWork()
    {
        ...
        // Need to hold handle until we submit work to GPU.
        auto& linearAllocator = DirectX::GraphicsMemory::Get(md3dDevice.Get());
        auto memHandle = linearAllocator.AllocateConstant(dispatchConstants);
        ...
        auto transition = D3D12::CD3DX12_RESOURCE_BARRIER::Transition(mOutputBuffer.Get(),
            D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COPY_SOURCE);
        mCommandList->ResourceBarrier(1, &transition);

        mCommandList->CopyResource(mReadBackBuffer.Get(), mOutputBuffer.Get());

        transition = D3D12::CD3DX12_RESOURCE_BARRIER::Transition(mOutputBuffer.Get(),
        ...
        auto mappedData = static_cast<Data*>(nullptr);
        ...
        for (int i = 0; i < NumDataElements; ++i)
        {
			fout << std::format("({}, {}, {}, {}, {})\n", i, mappedData[i].v1.x, mappedData[i].v1.y, mappedData[i].v1.z, mappedData[i].v2.x, mappedData[i].v2.y);
        }
...
private:
    ...
    Microsoft::WRL::ComPtr<D3D12::ID3D12Resource> mInputBufferA;
    Microsoft::WRL::ComPtr<D3D12::ID3D12Resource> mInputBufferB;
    Microsoft::WRL::ComPtr<D3D12::ID3D12Resource> mOutputBuffer;
    Microsoft::WRL::ComPtr<D3D12::ID3D12Resource> mReadBackBuffer;

    std::uint32_t mBufferIndexA = 0;
    std::uint32_t mBufferIndexB = 0;
    std::uint32_t mBufferOutputIndex = 0;
