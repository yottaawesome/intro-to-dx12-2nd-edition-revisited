...
export class StencilingApp : public D3DApp
{
public:
...
private:
    ...
    void OnKeyboardInput(const GameTimer& gt)
    {
        ...
        auto skullWorld = DirectX::XMMATRIX{ skullRotate * skullScale * skullOffset };
        DirectX::XMStoreFloat4x4(&mSkullRitem->World, skullWorld);
        ...
        DirectX::XMStoreFloat4x4(&mReflectedSkullRitem->World, skullWorld * R);

        // Update shadow world matrix.
        auto shadowPlane = DirectX::XMVECTOR{ DirectX::XMVectorSet(0.0f, 1.0f, 0.0f, 0.0f) }; // xz plane
        auto toMainLight = -DirectX::XMLoadFloat3(&mMainPassCB.gLights[0].Direction);
        ...
        DirectX::XMStoreFloat4x4(&mShadowedSkullRitem->World, skullWorld * S * shadowOffsetY);
    }
    ...
    void UpdateReflectedPassCB(const GameTimer& gt)
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            auto lightDir = DirectX::XMVECTOR{ DirectX::XMLoadFloat3(&mMainPassCB.gLights[i].Direction) };
            auto reflectedLightDir = DirectX::XMVECTOR{ DirectX::XMVector3TransformNormal(lightDir, R) };
            DirectX::XMStoreFloat3(&mReflectedPassCB.gLights[i].Direction, reflectedLightDir);
        }
    ...
    void BuildPSOs()
    {
        ...
        ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&transparentPsoDesc, __uuidof(D3D12::ID3D12PipelineState), &mPSOs["transparent"]));

        //
        // PSO for marking stencil mirrors.
        //

        auto mirrorBlendState = D3D12::CD3DX12_BLEND_DESC(D3D12::D3D12_DEFAULT);
        mirrorBlendState.RenderTarget[0].RenderTargetWriteMask = 0;

        auto mirrorDSS = D3D12::D3D12_DEPTH_STENCIL_DESC{
            .DepthEnable = true,
        ...
        auto markMirrorsPsoDesc = D3D12::D3D12_GRAPHICS_PIPELINE_STATE_DESC{ basePsoDesc };
        markMirrorsPsoDesc.BlendState = mirrorBlendState;
        markMirrorsPsoDesc.DepthStencilState = mirrorDSS;
        ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(
            &markMirrorsPsoDesc,
            __uuidof(D3D12::ID3D12PipelineState), &mPSOs["markStencilMirrors"]));

        //
        // PSO for stencil reflections.
        auto reflectionsDSS = D3D12::D3D12_DEPTH_STENCIL_DESC{ basePsoDesc.DepthStencilState };
        ...
        auto drawReflectionsPsoDesc = D3D12::D3D12_GRAPHICS_PIPELINE_STATE_DESC{ basePsoDesc };
        drawReflectionsPsoDesc.DepthStencilState = reflectionsDSS;
        drawReflectionsPsoDesc.RasterizerState.CullMode = D3D12::D3D12_CULL_MODE::D3D12_CULL_MODE_BACK;
        drawReflectionsPsoDesc.RasterizerState.FrontCounterClockwise = true;
        ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&drawReflectionsPsoDesc, __uuidof(D3D12::ID3D12PipelineState), &mPSOs["drawStencilReflections"]));
        ...
        auto shadowDSS = D3D12::D3D12_DEPTH_STENCIL_DESC{
            .DepthEnable = true,
        ...
        auto shadowPsoDesc = D3D12::D3D12_GRAPHICS_PIPELINE_STATE_DESC{transparentPsoDesc};
        shadowPsoDesc.DepthStencilState = shadowDSS;
        ThrowIfFailed(md3dDevice->CreateGraphicsPipelineState(&shadowPsoDesc, __uuidof(D3D12::ID3D12PipelineState), &mPSOs["shadow"]));
    }
    void BuildFrameResources()
    {
        auto& matLib = MaterialLib::GetLib();
        constexpr auto passCount = 2u;
    ...
    void BuildMaterials()
    {
        ...
        matLib.AddMaterial("checkertile",
            ...
            DirectX::XMFLOAT3(0.07f, 0.07f, 0.07f), 0.3f);

        matLib.AddMaterial("icemirror",
            texLib["iceMap"],
            texLib["defaultNormalMap"],
            texLib["defaultGlossHeightAoMap"],
            DirectX::XMFLOAT4(1.0f, 1.0f, 1.0f, 0.6f),
            DirectX::XMFLOAT3(0.1f, 0.1f, 0.1f), 0.5f);
        ...
        matLib.AddMaterial("shadowMat",
            ...
            DirectX::XMFLOAT3(0.01f, 0.01f, 0.01f), 0.0f);
    }
    ...
    ) -> std::unique_ptr<MeshGeometry>
    {
        ...
        auto vertices = std::array<ModelVertex, 20>{
        ...
        auto indices = std::array<std::uint16_t, 30>{
            // Floor
            0, 1, 2,
            0, 2, 3,
        ...
        auto floorSubmesh = SubmeshGeometry{
            .IndexCount = 6,
            .StartIndexLocation = 0,
            .BaseVertexLocation = 0
        };
        auto wallSubmesh = SubmeshGeometry{
            .IndexCount = 18,
            .StartIndexLocation = 6,
            .BaseVertexLocation = 0
        };
        auto mirrorSubmesh = SubmeshGeometry{
            .IndexCount = 6,
            .StartIndexLocation = 24,
            .BaseVertexLocation = 0
        };

        constexpr auto indexCount = (UINT)indices.size();
        constexpr auto indexElementByteSize = Win32::UINT{ sizeof(uint16_t) };
        constexpr auto vbByteSize = Win32::UINT{ vertices.size() * sizeof(ModelVertex) };
        constexpr auto ibByteSize = Win32::UINT{ indexCount * indexElementByteSize };

        const auto indexData = reinterpret_cast<Win32::byte*>(indices.data());
...
private:
    ...
    float mTheta = 1.4f * DirectX::Pi;
    float mPhi = 0.4f * DirectX::Pi;
    float mRadius = 12.0f;
