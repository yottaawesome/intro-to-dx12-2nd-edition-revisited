export module displacementmapping;
import std;
import shared;
...
export class DisplacementMappingApp : public D3DApp
{
public:
    DisplacementMappingApp(Win32::HINSTANCE hInstance)
        : D3DApp(hInstance)
    {
        Initialize();
    }
...
private:
    ...
    virtual void Update(const GameTimer& gt)override
    {
        ...
        //
        // Update tessellation parameters. Each object could have separate tessellation settings,
        // but in this demo they are all the same.
        //

        for (RenderItem* ri : mRitemLayer[(int)RenderLayer::OpaqueTess])
        {
            ri->MeshMinTessDist = mMinTessDistance;
            ri->MeshMaxTessDist = mMaxTessDistance;
            ri->MeshMinTess = 0;
            ri->MeshMaxTess = mMaxTess;
        }
    ...
    virtual void Draw(const GameTimer& gt)override
    {
        ...
        mCommandList->SetPipelineState(
            mDrawWireframe ?
            psoLib["opaque_wireframe"] :
            psoLib["opaque"]);
        DrawRenderItems(mCommandList.Get(), mRitemLayer[(int)RenderLayer::Opaque]);

        mCommandList->SetPipelineState(
            mDrawWireframe ?
            psoLib["opaque_tess_wireframe"] :
            psoLib["opaque_tess"]);
    ...
    virtual void UpdateImgui(const GameTimer& gt)override
    {
        ...
        ImGui::SliderFloat("Min Tess Distance", &mMinTessDistance, 0.0f, 50.0f);
        ImGui::SliderFloat("Max Tess Distance", &mMaxTessDistance, 0.0f, 100.0f);
        ImGui::SliderFloat("Max Tess", &mMaxTess, 0.0f, 6.0f);
        ImGui::SliderFloat("Displacement Scale", &mDisplacementScale, 0.0f, 0.5f);
    ...
    void AnimateMaterials(const GameTimer& gt)
    {
        ...
        for (RenderItem* ri : mRitemLayer[(int)RenderLayer::OpaqueTess])
        {
            auto m = static_cast<Material*>(ri->Mat);
            if (m->DisplacementScale != mDisplacementScale)
            {
                m->DisplacementScale = mDisplacementScale;
    ...
    void UpdatePerObjectCB(const GameTimer& gt)
    {
        ...
        for (auto& ri : mAllRitems)
        {
            ...
            ri->ObjectConstants.gMeshMinTessDist = ri->MeshMinTessDist;
            ri->ObjectConstants.gMeshMaxTessDist = ri->MeshMaxTessDist;
            ri->ObjectConstants.gMeshMinTess = ri->MeshMinTess;
            ri->ObjectConstants.gMeshMaxTess = ri->MeshMaxTess;
    ...
    )
    {
        ...
        ritem->World = world;
        ritem->TexTransform = texTransform;
        ritem->MeshMinTessDist = meshMinTessDist;
        ritem->MeshMaxTessDist = meshMaxTessDist;
        ritem->MeshMinTess = meshMinTess;
        ritem->MeshMaxTess = meshMaxTess;
        ritem->Mat = mat;
        ritem->Geo = geo;
        ritem->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_3_CONTROL_POINT_PATCHLIST;
        ritem->IndexCount = drawArgs.IndexCount;
        ritem->StartIndexLocation = drawArgs.StartIndexLocation;
        ritem->BaseVertexLocation = drawArgs.BaseVertexLocation;
    ...
    void BuildRenderItems()
    {
        ...
        const float meshMinTessDist = 5.0f;
        const float meshMaxTessDist = 25.0f;
        const float meshMinTess = 0.0f;
        const float meshMaxTess = 3.0f;

        worldTransform = MathHelper::Identity4x4;
        DirectX::XMStoreFloat4x4(&texTransform, DirectX::XMMatrixScaling(6.0f, 6.0f, 1.0f));
        AddDisplacementMappedRenderItem(
            ...
            mGeometries["shapeGeo"]->DrawArgs["grid"]);
        ...
        for (auto i = 0; i < 5; ++i)
        {
            ...
            AddRenderItem(RenderLayer::Opaque, worldTransform, texTransform,
                matLib["columnSquare"],
                mGeometries[columnNameLeft].get(),
                mGeometries[columnNameLeft]->DrawArgs["subset0"]);

            DirectX::XMStoreFloat4x4(&texTransform, DirectX::XMMatrixScaling(1.0f, 1.0f, 1.0f));
            DirectX::XMStoreFloat4x4(&worldTransform, DirectX::XMMatrixTranslation(+5.0f, 0.0f, -10.0f + i * 5.0f));
            AddRenderItem(RenderLayer::Opaque, worldTransform, texTransform,
                matLib["columnSquare"],
                mGeometries[columnNameRight].get(),
                mGeometries[columnNameRight]->DrawArgs["subset0"]);
