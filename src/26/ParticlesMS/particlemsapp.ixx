...
{
public:
    ParticlesMsApp(Win32::HINSTANCE hInstance)
        : D3DApp(hInstance)
    {
...
private:
    ...
    void Update(const GameTimer& gt)override
    {
        ...
        mHelixParticleConstants.gHelixColorTint = DirectX::XMFLOAT4(0.5f, 0.5f, 0.5f, 0.3f);

        mHelixParticleConstants.gHelixOrigin = DirectX::XMFLOAT3(0.0f, 0.0f, 0.0f);
        mHelixParticleConstants.gHelixParticleCount = TotalParticles;

        mHelixParticleConstants.gHelixRadius = 4.0f;
        mHelixParticleConstants.gHelixHeight = 6.0f;

        constexpr float numOscillations = 4.0f;
        mHelixParticleConstants.gHelixAngularFrequency = 2.0f * MathHelper::Pi * numOscillations;
        mHelixParticleConstants.gHelixParticleSize = 1.0f;
        mHelixParticleConstants.gHelixSpeed = 0.1f;

        TextureLib& texLib = TextureLib::GetLib();
        mHelixParticleConstants.gHelixTextureId = texLib["explosionParticle"]->BindlessIndex;
    }
    ...
    void Draw(const GameTimer& gt)override
    {
        ...
        DrawHelixParticles(mCommandList.Get());
    ...
    {
        auto cbHandle = mLinearAllocator->AllocateConstant(mHelixParticleConstants);
        cmdList->SetGraphicsRootConstantBufferView(GFX_ROOT_ARG_OBJECT_CBV, cbHandle.GpuAddress());

        auto& psoLib = PsoLib::GetLib();
        cmdList->SetPipelineState(psoLib["helixParticles_ms"]);
        cmdList->DispatchMesh(NumMeshShaderGroups, 1, 1);
    }
